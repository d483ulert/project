...
public class BoardController {
	...
	public String boardWrite1(BoardVO vo) throws Exception{
		bs.write(vo);
		return "redirect:/board/list";  // redirect?óê?äî /board/listÍπåÏ? ?†Å?ñ¥?ïº?ï® ÏµúÏÉÅ?úÑ RequestMapping valueÍπåÏ??è¨?ï®?ïò?ó¨. 
	}
	
	@RequestMapping(value="/read")
	public String boardRead(BoardVO vo, Model model,@RequestParam int bno) throws Exception{
		BoardVO list= bs.getRead(bno);
		model.addAttribute("board",list); //Don't know how to iterate over supplied "items" in &lt;forEach&gt; listÍ∞??ïÑ?ãå?ç∞ listÎ°? Î≥¥ÎÇ¥Ï§òÏÑú ?ÉùÍ∏¥Î¨∏?†ú
